using Workflow.Tests.Validation.Checker.Core;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using Workflow.Tests.Validation.Checker.Checkers;
using System.IO;
using System;
...
namespace Workflow.Tests.Validation
{
    ...
    public class DefenderTests
    {
        ...
        {
            string path = PluginLoader.GetPluginPath("Workflow.Models");
            Assert.IsFalse(ScanPath(path));
        }
        [TestMethod]
        public void ScanAgentManagersWindows()
        {
            string path = PluginLoader.GetPluginPath("Workflow.Providers.Windows");
            Assert.IsFalse(ScanPath(path));
        }
        [TestMethod]
        public void ScanAgentManagersReflection()
        {
            string path = PluginLoader.GetPluginPath("Workflow.Providers.Runtime");
            Assert.IsFalse(ScanPath(path));
        }
        [TestMethod]
        public void ScanAgentProfilesHttp()
        {
            string path = PluginLoader.GetPluginPath("Workflow.Channels.Http");
            Assert.IsFalse(ScanPath(path));
        }
        [TestMethod]
        public void ScanAgentProfilesSmb()
        {
            string path = PluginLoader.GetPluginPath("Workflow.Channels.Smb");
            Assert.IsFalse(ScanPath(path));
        }
        [TestMethod]
        public void ScanAgentProfilesWebsocket()
        {
            string path = PluginLoader.GetPluginPath("Workflow.Channels.Websocket");
            Assert.IsFalse(ScanPath(path));
        }
