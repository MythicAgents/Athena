...
namespace IronPython.Runtime
{
    ...
    /// </summary>
    [Serializable]
    internal sealed class CustomMemoryStreamContentProvider : TextContentProvider
    {
        ...
        internal CustomMemoryStreamContentProvider(PythonContext context, byte[] data, string path)
            : this(context, data, 0, data?.Length ?? 0, path) { }

        internal CustomMemoryStreamContentProvider(PythonContext context, byte[] data, int index, int count, string path)
        {
