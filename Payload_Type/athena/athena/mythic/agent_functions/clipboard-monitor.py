...
class ClipboardMonitorCommand(CommandBase):
    ...
    description = "Monitor clipboard for changes (Windows only, long-running job)"
    version = 1
    author = "@checkymander"
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        ...
        response.DisplayParams = f"{taskData.args.get_arg('duration')}s every {taskData.args.get_arg('interval')}s"
        return response
